export const data = [
   {
      "main": [1,4],
      "name": "Baked Potato",
      "procedure": [
         "Gather all ingredients.",
         "In a small bowl, mix mayonnaise, garlic, oregano, salt , and pepper. Set aside.",
         "Bring a large pot of salted water to a boil. Add potatoes, and cook until almost done, about 10 minutes. Don’t overcook otherwise the potatoes will break apart. Drain, and cool.",
         "Preheat oven broiler. Line a baking tray with aluminum foil, and lightly grease the aluminum foil.",
         "Arrange potatoes in the prepared baking tray. Spoon the mayonnaise mixture over the potatoes.",
         "Broil in the preheated oven until potatoes are tender and mayonnaise mixture is lightly browned, about 10 minutes.",
         "Serve hot and enjoy!"
      ],
      "ingredients": [
         "3 tablespoons mayonnaise",
         "2 cloves garlic, crushed",
         "1 teaspoon dried oregano",
         "Salt and pepper to taste",
         "5 potatoes, quartered"
      ],
      "preparationTime": 20,
      "pic": require("./bakedpotato.jpg")
   },
   {
      "main": [1,3],
      "name": "Cheesy Potato",
      "procedure": [
         "Gather all ingredients.",
         "Mix potatoes, condensed soup, and sour cream together in a large baking dish; mix in cheese, butter, salt, and pepper to taste. Top mixture with potato chips.",
         "Bake in the preheated oven until the mixture is bubbly and lightly brown, about 1 hour.",
         "Remove from oven and let stand 15 minutes before serving."
      ],
      "ingredients": [
         "Preheat the oven to 350 degrees F (175 degrees C).",
         "1 (10.75 ounce) can condensed cream of potato soup",
         "1 (16 ounce) container sour cream",
         "1 (16 ounce) package shredded Cheddar cheese",
         "½ cup butter, melted",
         "1 cup crushed potato chips"
      ],
      "preparationTime": 90,
      "pic": require("./potatocosserole.webp")
   },
   {
      "main": [1],
      "name": "French Fries",
      "procedure": [
         "Soak potato strips in a large bowl of water for about 30 minutes.",
         "Pat with paper towels until thoroughly dry.",
         "Heat oil in a deep-fryer or large saucepan to 275 degrees F (135 degrees C). Gently add potatoes to the hot oil and fry for about 5 minutes, stirring and flipping the potatoes occasionally.",
         "Use a slotted spoon to transfer potatoes to a paper towel-lined plate. Let cool completely.",
         "Heat oil again, but this time to 350 degrees F (175 degrees C). Add potatoes and fry a second time until golden brown, 5 to 6 minutes.",
         "Remove from the deep-fryer and blot with a paper towel. Sprinkle with salt to serve."
      ],
      "ingredients": [
         "1 large russet potato, cut into evenly sized strips",
         "Vegetable oil for frying, about 2 cups or as needed",
         "salt to taste"
      ],
      "preparationTime": 20,
      "pic": require("./frenchfries.webp")
   },
   {
      "main": [1,3,4],
      "name": "Garlic Potatoes Gratin",
      "procedure": [
         "Preheat oven to 325 degrees F (165 degrees C). Lightly grease a 9x13 inch baking dish.",
         "In the prepared dish layer 1/2 of the potatoes, 1/2 of the cheese, then the remaining potatoes.",
         "Melt the butter in a small skillet over medium heat. Saute garlic until fragrant and golden brown; pour over potatoes.",
         "Combine cream, salt and pepper; pour evenly over potatoes and sprinkle with remaining cheese.",
         "Bake in preheated oven for 75 minutes. Serve immediately."
      ],
      "ingredients": [
         "3 pounds red potatoes, peeled and sliced",
         "6 ounces Gouda cheese, shredded, divided",
         "3 tablespoons butter",
         "5 cloves garlic, minced",
         "1 ½ cups heavy cream",
         "1 teaspoon salt",
         "½ teaspoon black pepper"
      ],
      "preparationTime": 90,
      "pic": require("./cheesegarlicpotato.jpg")
   },
   {
      "main": [1,5],
      "name": "Chicken Pot Pie",
      "procedure": [
         "Preheat the oven to 425 degrees F (220 degrees C).",
         "Melt butter in a large skillet over medium heat. Cook and stir onion, flour, salt, and pepper in hot butter until onion is translucent, about 5 minutes.",
         "Pour in broth and milk; bring to a boil and cook until slightly thickened, about 1 minute.",
         "Press 1 pie pastry into the bottom of a 9-inch, deep-dish pie pan. Pour chicken mixture into pie pastry.",
         "Top with remaining pastry and press edges together to seal. Cut several slits in top pastry. Place pie on a baking sheet.",
         "Bake in the preheated oven until crust is golden brown, about 30 minutes."
      ],
      "ingredients": [
         "⅓ cup butter",
         "⅓ cup chopped onion",
         "⅓ cup all-purpose flour",
         "½ teaspoon salt",
         "¼ teaspoon ground black pepper",
         "2 cups chopped cooked chicken",
         "½ (15 ounce) can whole new potatoes, drained"
      ],
      "preparationTime": 45,
      "pic": require("./chickenpotpie.webp")
   },
   {
      "main": [1,2],
      "name": "Potato Broccoli Soup",
      "procedure": [
         "Gather all ingredients.",
         "In a large pot, combine potatoes, broccoli, and chicken broth. Bring to a boil, then reduce heat and simmer until potatoes are tender, about 15 minutes.",
         "In a separate saucepan, melt butter over medium heat. Stir in flour and cook for 1 minute.",
         "Gradually stir in milk and cook until thickened. Add cheese and stir until melted.",
         "Combine the cheese mixture with the potato mixture. Season with salt and pepper to taste.",
         "Serve hot and enjoy!"
      ],
      "ingredients": [
         "3 cups diced potatoes",
         "2 cups chopped broccoli",
         "4 cups chicken broth",
         "3 tablespoons butter",
         "3 tablespoons all-purpose flour",
         "2 cups milk",
         "1 cup shredded Cheddar cheese",
         "Salt and pepper to taste"
      ],
      "preparationTime": 30,
      "pic": require("./potatobroccolisoup.webp")
   },
   {
      "main": [2,5],
      "name": "Chicken and Broccoli Casserole",
      "procedure": [
         "Preheat the oven to 350 degrees F (175 degrees C).",
         "In a large bowl, mix together the chicken, broccoli, condensed soup, and milk. Season with salt and pepper to taste.",
         "Spread the mixture into a 9x13 inch baking dish. Top with shredded cheese and bread crumbs.",
         "Bake in the preheated oven until bubbly and golden brown, about 25 minutes.",
         "Serve hot and enjoy!"
      ],
      "ingredients": [
         "2 cups cooked and diced chicken",
         "2 cups chopped broccoli",
         "1 (10.75 ounce) can condensed cream of mushroom soup",
         "1/2 cup milk",
         "1 cup shredded Cheddar cheese",
         "1/2 cup bread crumbs",
         "Salt and pepper to taste"
      ],
      "preparationTime": 40,
      "pic": require("./chickenbroccolicasserole.webp")
   },
   {
      "main": [1,2,4],
      "name": "Garlic Potato Broccoli Mash",
      "procedure": [
         "Bring a large pot of salted water to a boil. Add potatoes and cook until tender, about 15 minutes. Drain and return to the pot.",
         "Steam broccoli until tender, about 5 minutes.",
         "In a small skillet, melt butter over medium heat. Add garlic and cook until fragrant, about 1 minute.",
         "Add the steamed broccoli and garlic butter to the potatoes. Mash together until smooth. Season with salt and pepper to taste.",
         "Serve hot and enjoy!"
      ],
      "ingredients": [
         "4 large potatoes, peeled and cubed",
         "2 cups chopped broccoli",
         "3 tablespoons butter",
         "3 cloves garlic, minced",
         "Salt and pepper to taste"
      ],
      "preparationTime": 30,
      "pic": require("./garlicpotatobroccolimash.jpeg")
   },
   {
      "main": [1, 3, 5],
      "name": "Cheesy Chicken Potato Skillet",
      "procedure": [
         "Heat oil in a large skillet over medium-high heat.",
         "Add chicken and cook until browned, about 5 minutes.",
         "Add potatoes and cook until tender, about 15 minutes.",
         "Stir in cheese and cook until melted.",
         "Season with salt and pepper to taste.",
         "Serve hot and enjoy!"
      ],
      "ingredients": [
         "2 tablespoons vegetable oil",
         "1 pound boneless, skinless chicken breasts, cut into strips",
         "4 cups diced potatoes",
         "2 cups shredded Cheddar cheese",
         "Salt and pepper to taste"
      ],
      "preparationTime": 30,
      "pic": require("./cheesychickenpotatoskillet.webp")
   },
   {
      "main": [1, 4, 5],
      "name": "Garlic Chicken Potato Bake",
      "procedure": [
         "Preheat the oven to 375 degrees F (190 degrees C).",
         "In a large bowl, mix together potatoes, garlic, and chicken. Season with salt and pepper to taste.",
         "Spread the mixture into a 9x13 inch baking dish.",
         "Bake in the preheated oven until bubbly and golden brown, about 45 minutes.",
         "Serve hot and enjoy!"
      ],
      "ingredients": [
         "4 cups diced potatoes",
         "4 cloves garlic, minced",
         "1 pound boneless, skinless chicken breasts, cut into strips",
         "Salt and pepper to taste"
      ],
      "preparationTime": 60,
      "pic": require("./garlicchickenpotatobake.webp")
   },
   {
      "main": [2, 3, 4],
      "name": "Cheesy Garlic Broccoli Bake",
      "procedure": [
         "Preheat the oven to 375 degrees F (190 degrees C).",
         "In a large bowl, mix together broccoli, garlic, and cheese. Season with salt and pepper to taste.",
         "Spread the mixture into a 9x13 inch baking dish.",
         "Bake in the preheated oven until bubbly and golden brown, about 30 minutes.",
         "Serve hot and enjoy!"
      ],
      "ingredients": [
         "4 cups chopped broccoli",
         "4 cloves garlic, minced",
         "2 cups shredded Cheddar cheese",
         "Salt and pepper to taste"
      ],
      "preparationTime": 45,
      "pic": require("./cheesygarlicbroccolibake.jpg")
   },
   {
      "main": [2, 3, 5],
      "name": "Cheesy Chicken Broccoli Bake",
      "procedure": [
         "Preheat the oven to 375 degrees F (190 degrees C).",
         "In a large bowl, mix together broccoli, chicken, and cheese. Season with salt and pepper to taste.",
         "Spread the mixture into a 9x13 inch baking dish.",
         "Bake in the preheated oven until bubbly and golden brown, about 30 minutes.",
         "Serve hot and enjoy!"
      ],
      "ingredients": [
         "4 cups chopped broccoli",
         "1 pound boneless, skinless chicken breasts, cut into strips",
         "2 cups shredded Cheddar cheese",
         "Salt and pepper to taste"
      ],
      "preparationTime": 45,
      "pic": require("./cheesychickenbroccolibake.png")
   },
   {
      "main": [2, 4, 5],
      "name": "Garlic Chicken Broccoli Stir-Fry",
      "procedure": [
         "Heat oil in a large skillet over medium-high heat.",
         "Add chicken and cook until browned, about 5 minutes.",
         "Add garlic and cook until fragrant, about 1 minute.",
         "Stir in broccoli and cook until tender, about 5 minutes.",
         "Season with salt and pepper to taste.",
         "Serve hot and enjoy!"
      ],
      "ingredients": [
         "2 tablespoons vegetable oil",
         "1 pound boneless, skinless chicken breasts, cut into strips",
         "4 cloves garlic, minced",
         "4 cups chopped broccoli",
         "Salt and pepper to taste"
      ],
      "preparationTime": 20,
      "pic": require("./garlicchickenbroccolistirfry.jpeg")
   },
   {
      "main": [3, 4, 5],
      "name": "Cheesy Garlic Chicken Skillet",
      "procedure": [
         "Heat oil in a large skillet over medium-high heat.",
         "Add chicken and cook until browned, about 5 minutes.",
         "Add garlic and cook until fragrant, about 1 minute.",
         "Stir in cheese and cook until melted.",
         "Season with salt and pepper to taste.",
         "Serve hot and enjoy!"
      ],
      "ingredients": [
         "2 tablespoons vegetable oil",
         "1 pound boneless, skinless chicken breasts, cut into strips",
         "4 cloves garlic, minced",
         "2 cups shredded Cheddar cheese",
         "Salt and pepper to taste"
      ],
      "preparationTime": 20,
      "pic": require("./cheesygarlicchickenskillet.jpeg")
   }
];
